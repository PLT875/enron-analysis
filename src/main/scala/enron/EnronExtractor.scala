 ...
 */
case class EmailMessage(to: Option[String], cc: Option[String], lines: Option[List[String]])
...
object EnronExtractor {
   ...
   */
  def getMessageLines(fileList: NodeList, zip: ZipFile): Option[List[String]] = {
    ...
    val lines = Stream.continually(br.readLine).takeWhile(_ != null).toList
